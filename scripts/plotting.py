...
def loss_per_country(path_to_summary_excel,figure_output_path):
    '''
    PLOT PER YEAR PER COUNTRY
    '''
    ...
    all_storm = pd.read_excel('output_storms.xlsx',sheetname='total_losses') #,index_col=0)
    ...
    ax_yc = loss_per_year.plot.bar(stacked=True,width=0.9, ec="w", lw=0.1,colormap="Paired")
...
def loss_per_sector(path_to_summary_excel,figure_output_path):
    '''
    PLOT PER YEAR PER SECTOR
    '''
    ...
    ax_ys = loss_per_sector.plot.bar(stacked=True,width=0.9, ec="w", lw=0.1,colormap="Paired")
